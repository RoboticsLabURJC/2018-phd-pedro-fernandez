...
for episode in range(EPISODES):
    ...
    if not episode % STATS_EVERY:
        ...
        #print(f'Learning Rate: {learningrates}, episode: {episode:>5d}, average reward: {average_reward:>4.1f}, min reward: {min(ep_rewards[-STATS_EVERY:])}, max reward: {max(ep_rewards[-STATS_EVERY:])},current epsilon: {epsilon:>1.2f}')

...
for episode in range(EPISODES):
    ...
    if not episode % STATS_EVERY:
        ...
        print(f'Learning Rate: {LEARNING_RATE}, discount: {DISCOUNT:>2.2f}, episode: {episode:>5d}, average reward: {average_reward:>4.1f}\
,min reward: {min(ep_rewards[-STATS_EVERY:])}, max reward: {max(ep_rewards[-STATS_EVERY:])}\
,current epsilon: {epsilon:>1.3f},epsilon_decay_value: {epsilon_decay_value:>1.5f}')        #print(f'Learning Rate: {learningrates}, episode: {episode:>5d}, average reward: {average_reward:>4.1f}, min reward: {min(ep_rewards[-STATS_EVERY:])}, max reward: {max(ep_rewards[-STATS_EVERY:])},current epsilon: {epsilon:>1.2f}')


    # lets save results in a table
    if episode % SAVE_TABLE_EVERY == 0 and episode > 0:
        np.save(f"qtables/SARSA-{LEARNING_RATE}-{DISCOUNT}-{episode}-qtable.npy", q_table)
...
plt.title('SARSA with Learning Rate and Discount Rate')

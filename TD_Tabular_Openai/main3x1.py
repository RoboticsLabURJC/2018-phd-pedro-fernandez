...
Main3x1 stands for:
    ...
    - 1 hyperparameter: alpha or step size or Learning rate which runs 10 times (or more) for each algorithm 


Very easy to change step size by Gamma or another hyperparam
...
RESULTS
    ...
    - 10_000 episodes and 10 Learning Rates took 7394 secs or 124 min. aprox
...
best_stats = {
    'QLearning': {
        'episode': [], 'learning_rate': [], 'best_value': []},
     'SARSA': {
        'episode': [], 'learning_rate': [], 'best_value': []},
    'expected SARSA': {
        'episode': [], 'learning_rate': [], 'best_value': []}
...
for agent in tqdm(agents):
    for step_size in tqdm(params.LEARNING_RATES):
        ...
        if (agent, step_size) not in best_stepsize:
            best_stepsize[(agent, step_size)] = []  
...
print("------------- Stats for debug purposes -----------------")
print(f"Q-Learning best Return: {QLearning_stats['best_value']} in episode: {QLearning_stats['episode']} with step size(learning rate): {QLearning_stats['learning_rate']}")
print(f"SARSA best Return: {SARSA_stats['best_value']} in episode: {SARSA_stats['episode']} with step size(learning rate): {SARSA_stats['learning_rate']}")
print(f"expected SARSA best Return: {expectedSARSA_stats['best_value']} in episode: {expectedSARSA_stats['episode']} with step size(learning rate): {expectedSARSA_stats['learning_rate']}")


# --------- Saving Results 
os.makedirs("Files", exist_ok = True)             
QLearning_file = open(f"Files/QLearning_stats-episodes({params.EPISODES})-alphas({len(params.LEARNING_RATES)})-{time.strftime('%Y%m%d-%H%M%S')}.csv", "a")
SARSA_file = open(f"Files/SARSA_stats-episodes({params.EPISODES})-alphas({len(params.LEARNING_RATES)})-{time.strftime('%Y%m%d-%H%M%S')}.csv", "w")
expected_SARSA_file = open(f"Files/expected_SARSA_stats-episodes({params.EPISODES})-alphas({len(params.LEARNING_RATES)})-{time.strftime('%Y%m%d-%H%M%S')}.csv", "w")
#a_dict = {"a": 1, "b": 2}
